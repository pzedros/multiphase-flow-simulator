...
def pressao_bolha(RGL, dg, T, API):
    """
    Pressão de Bolha - Correlação de Standing (1947).
    Entradas:
    - RGL: Razão Gás-Óleo na superfície [SCF/STB ou sm³/sm³]
    - dg: Gravidade específica do gás [adimensional]
    - T: Temperatura [°C] (convertida internamente para °F)
...
def rs_gas_oil(dg, Pbar, Pb, API, T):
    """
    Razão de Solubilidade Gás-Óleo (Rs) - Correlação de Standing.
    Entradas:
    - dg: Gravidade específica do gás [adimensional]
    - Pbar: Pressão atual [bar] (convertida internamente para psia)
    - Pb: Pressão de bolha [psia]
    - API: Grau API do óleo [°API]
    - T: Temperatura [°C] (convertida internamente para °F)
    Saída:
