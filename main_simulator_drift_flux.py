...
def simular(D_m, sections):
    ...
    for step in sections:
        ...
        Vsl = q_liq_insitu / Area
        Vsg = q_gas_insitu / Area
        ...
        # ---- Drift Flux (Bendiksen, 1984) ----
        resultado = df_model.calcular(
            vsl=Vsl, vsg=Vsg,
            rho_l=rho_liq, rho_g=rho_g_si,
            mu_l=mu_liq,   mu_g=mu_g_Pas,
            Dh=D_m, g=g_accel, theta=theta, eps=eps
        )
        ...
        vd      = resultado["vd"]
        Vm      = resultado["vm"]
        rho_mix = resultado["rho_m"]

        dPdL_T = resultado["dPdL"]
        dPdL_F = resultado["dPdL_f"]
        dPdL_G = resultado["dPdL_g"]
        ...
        dados["rho_mix"].append(rho_mix)
        dados["mu_o"].append(mu_o_Pas)
        dados["mu_g"].append(mu_g_Pas)
        dados["mu_mix"].append(mu_liq * HL + mu_g_Pas * (1.0 - HL))
...
for D_pol in DIAMETROS_POL:
    ...
    resultados[D_pol] = df
    df.to_excel(f"Relatorio_DF_{D_pol}pol.xlsx", index=False)
...
def plotar(var, titulo, ylabel):
    ...
    fig.tight_layout()
    nome = var.replace("/", "_")
    fig.savefig(f"DF_{nome}.png", dpi=150)
    plt.close(fig)
...
plt.title("Pressão ao longo do sistema — Drift Flux", fontsize=14)
...
plotar("C0",      "Parâmetro de Distribuição C₀",      "C₀ (-)")
plotar("vd",      "Velocidade Drift vd",               "vd (m/s)")
...
plotar("rho_mix", "Densidade da Mistura (drift)",      "kg/m³")
